...
AS CustomerCount 
FROM orders; -- Answer 89

-- 5.Write a query against the orders table that displays the total number
-- of distinct customers who have placed orders, by customer ID, for each country where orders have been shipped. 
...
-- but 1 or more units of them are on order? Write a query that orders them by quantity on order 
-- (high to low), then by product name.
SELECT productid, productname, unitsinstock, unitsonorder 
FROM products 
WHERE unitsinstock < 25 
	and Unitsonorder >=1 
		ORDER BY quantityperunit DESC, Productname DESC;

-- 7.Write a query to list each of the job titles in employees, along with a 
-- count of how many employees hold each job title.
SELECT title,
