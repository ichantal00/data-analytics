...
SELECT productname, MIN(unitprice) FROM products GROUP BY;

-- 2.Write a query to find the average price of all items that Northwind sells. 
-- (Bonus: Once you have written a working query, try asking Claude or ChatGPT for 
-- help using the ROUND function to round the average price to the nearest cent.)

-- 3.Write a query to find the price of the most expensive item that Northwind sells. 
-- Then write a second query to find the name of the product with that price, plus the 
-- name of the supplier for that product.
...
-- 7.Write a query to find the list of all category names and the average price for 
-- items in each category.

-- 8.Write a query to find, for all suppliers that provide at least 5 items to 
-- Northwind, what is the name of each supplier and the number of items they supply.
...
-- Sort the results in descending order by value. If two or more have the same value, order by
-- product name. If a product is not in stock, leave it off the list.